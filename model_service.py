...
def get_models():
    """Load the bundled model artifacts.

    Deployment intentionally does NOT fall back to MLflow. The registry database
    contains machine-specific/local artifact URIs and is not a portable model
    source for Streamlit Cloud.
    """
    ...
    if not os.path.exists(bin_path):
        missing.append("models/binary_model.joblib")
    if not os.path.exists(mc_path):
        missing.append("models/multiclass_model.joblib")

    # RUL model is split into GitHub-safe chunks. Load it in memory so the app
    # does not need write permission on Streamlit Cloud's source directory.
    rul_model = None
    if os.path.exists(rul_path):
        ...
        parts = []
        part_no = 1
        while os.path.exists(f"{rul_path}.part{part_no}"):
            parts.append(f"{rul_path}.part{part_no}")
            part_no += 1
        ...
        if parts:
            ...
            buffer.seek(0)
            rul_model = joblib.load(buffer)
        else:
            missing.append("models/rul_model.joblib or .part1/.part2/...")

    if missing:
        raise FileNotFoundError(
            "Required model files are missing from the deployment: "
            + ", ".join(missing)
        )

    _MODELS_CACHE["binary"] = joblib.load(bin_path)
    _MODELS_CACHE["rul"] = rul_model
