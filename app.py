...
# ==============================================================================
    ...
    if not MLFLOW_DB.exists():
        ...
        # Read the committed MLflow SQLite snapshot directly. Do not initialize
        # an MLflow client here: the database was created in another environment
        # and its artifact URIs point to local training-machine paths.
        st.info(
            "Showing the committed MLflow tracking snapshot. "
            "Model inference uses the portable joblib files in `models/`."
        )

        try:
            conn = sqlite3.connect(MLFLOW_DB.as_posix())
            ...
            for exp_name in experiment_names:
                match = exp_df[exp_df["name"] == exp_name]
                if match.empty:
                    continue
                ...
                if runs.empty:
                    st.caption(f"No runs logged for `{exp_name}`.")
                    continue

                view = (
                    runs.pivot_table(
                        index=["run_id", "runName"],
                        columns="key",
                        values="value",
                        aggfunc="last",
                    )
                ...
                preferred = [
                    ...
                    "test_f1", "test_recall_macro", "test_f1_macro",
                    "test_mae", "test_rmse", "test_r2",
                ]
                available = [c for c in preferred if c in view.columns]
                display_cols = ["runName"] + available

                st.markdown(f"### Experiment: `{exp_name}`")
                st.dataframe(
                    view[display_cols].sort_values("runName"),
                ...
                if "failure_24h" in exp_name:
                    ...
                    metric_cols = [c for c in [
                        "test_accuracy", "test_recall_macro", "test_f1_macro"
                    ] if c in view.columns]
                else:
                    metric_cols = [c for c in [
                        "test_mae", "test_rmse", "test_r2"
                ...
                if metric_cols:
                    ...
                    fig = px.bar(
                        ...
                        color="Metric",
                        barmode="group",
                        title=f"MLflow Metric Benchmarks: {exp_name}",
                    )
                    st.plotly_chart(
                        fig_layout(fig, 390),
                        use_container_width=True,
            ...
            st.markdown("### Registered Models (Snapshot)")
            reg_names = [
            ...
            for name in reg_names:
                versions = pd.read_sql_query(
                    """
                    SELECT name, version, current_stage, run_id, status
                    FROM model_versions
                    WHERE name = ?
            ...
            st.dataframe(
                pd.DataFrame(
                    reg_rows,
                    columns=["Registered Model", "Version", "Stage", "Run ID", "Status"],
                ),
                use_container_width=True,
            )
            ...
            st.error(
                "The MLflow database snapshot could not be read. "
                "Live predictions are independent of this database."
            )
            st.caption(f"Database read error: {type(exc).__name__}: {exc}")
